...
func NewPrometheus(subsystem string, notTemplates []string) *Prometheus {
	...
	p := &Prometheus{
		...
		ReqCntURLLabelMappingFn: func(c *gin.Context) string {
			...
			for _, p := range c.Params {
				i := 0
				if len(notTemplates) == 0 {
					url = strings.Replace(url, p.Value, ":"+p.Key, 1)
				}
				
				for _, notTemplate := range notTemplates {
					if p.Key == notTemplate {
						break
					} else {
						i++
						if i == len(notTemplates) {
							url = strings.Replace(url, p.Value, ":"+p.Key, 1)
						}
					}
				}
